...
export function GroupList({
  ...
  const listKey = kind === "party" ? "/api/parties" : "/api/clans";
  const dataKey = kind === "party" ? "parties" : "clans";
  const detailBase = kind === "party" ? "/parties" : "/clans";
  const levelProgress = kind === "party" ? partyLevelProgress : clanLevelProgress;

  const { data, mutate } = useSWR<Record<string, GroupSummary[]>>(listKey, fetcher);
  const groups = data?.[dataKey] ?? [];
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await postJson(listKey, { name: name.trim(), goal: goal.trim() || undefined });
      setName("");
      setGoal("");
      setShowCreate(false);
      await mutate();
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await postJson(`${listKey}/join`, { inviteCode: inviteCode.trim() });
      setInviteCode("");
      setShowJoin(false);
      await mutate();
    } catch (err) {
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {groups.map((g) => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-slate-500">멤버 {g.members.length}명</p>
            <div className="mt-3">
