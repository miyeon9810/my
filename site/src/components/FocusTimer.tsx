...
import { listenActiveSession, listenRecentSessions, startFocusSession, stopFocusSession } from "@/lib/focus-service";
import type { FocusSession, Group } from "@/types/game";
...
export function FocusTimer({
  ...
  useEffect(() => {
    if (!user) return;
    const un1 = listenActiveSession(user.uid, setActive);
    const un2 = listenRecentSessions(user.uid, setRecent);
    return () => {
      un1();
      un2();
    };
  ...
  useEffect(() => {
    if (!active) return;
    const start = active.startedAt.getTime();
  ...
  async function handleStart() {
    ...
    try {
      ...
      await startFocusSession(user.uid, {
  ...
  async function handleStop() {
    ...
    const result = await stopFocusSession(user.uid, active.id);
    showReward(result);
  }
    ...
    <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-5">
      <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
        <div className="text-center sm:text-left">
          <p className="text-xs text-slate-500">{active ? "집중 중이야" : "집중 타이머를 시작해봐"}</p>
          ...
          <ul className="space-y-1.5 text-sm">
            {recent.slice(0, 5).map((s) => (
              <li key={s.id} className="flex items-center justify-between text-slate-400">
                <span>
                  {s.startedAt.toLocaleDateString("ko-KR")}
                  {s.partyId && " · 🤝 파티"}
                  {s.clanId && " · 🏰 클랜"}
                </span>
                <span className="tabular-nums text-slate-300">
                  {formatDuration(s.durationSeconds)} <span className="text-amber-400">+{s.xpEarned}XP</span>
                </span>
              </li>
            ))}
          </ul>
